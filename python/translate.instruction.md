...
- **占位符必须原样保留（强约束）**：若原文包含占位符（如 `%s`、`%d`、`%02d`、`{name}` 等），译文中必须**一字不改地保留**该占位符，不可翻译、不可替换、不可删除或移动；仅翻译占位符前后的自然语言部分。程序会依赖这些占位符做插值，一旦缺失或错位会导致游戏显示错误。
...
- [ ] 译文长度 == 原文长度（字符数）
- [ ] 本批内术语、人称、风格一致
- [ ] 控制符/占位符完整且未损坏（含 `%s`、`%d`、`{name}` 等必须原样保留）
- [ ] 风格符合官方中文火影（简洁自然）
